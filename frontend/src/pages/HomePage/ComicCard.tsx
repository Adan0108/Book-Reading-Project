...
const ComicCard = ({ title, imageUrl, rating, viewCount, chapters, isHot = false, variant = 'default' }: ComicCardProps) => {
  ...
  const renderStars = (rate: number) => {
    return (
      <div className="flex items-center space-x-0.5 text-yellow-400">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg key={star} className={`w-3 h-3 ${star <= rate ? 'fill-current' : isDark ? 'text-gray-600' : 'text-gray-300'}`} viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
        <span className= {`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}   ml-1 font-medium`}>{rate}</span>
      </div>
    );
  };
  ...
  const containerClass = isTrending
    ...
    : `group flex flex-row rounded-lg overflow-hidden h-40 sm:h-48 shadow-sm hover:shadow-md transition-shadow border ${
        isDark 
  ...
  return (
    <div className= {containerClass}>
      ...
      <div className = "flex flex-col flex-grow p-3 sm:p-4 min-w-0">
        ...
        <a
          href="#" 
          className= {`text-base sm:text-lg font-bold ${isDark ? 'text-gray-100' : 'text-gray-800'} hover:text-yellow-500 line-clamp-1 sm:line-clamp-2 leading-tight mb-2 transition-colors`}
          title={title}
        >
          {title}
        </a>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-3 text-xs">
          {renderStars(rating)}
          <span className= {`${isDark ? 'text-gray-600' : 'text-gray-300'} hidden sm:inline`}>•</span>
          <span className= {`px-2 py-0.5 ${isDark ? 'bg-gray-700 text-gray-300 border-gray-600' : 'bg-gray-100  text-gray-600 border-gray-200'} rounded text-[10px] sm:text-[11px] font-medium border`}>
            {viewCount} lượt xem
          </span>
        </div>
        ...
        <div className = "mt-auto space-y-1">
          {chapters.slice(0, 3).map((chap) => (
            <div key={chap.id} className="flex items-center justify-between text-xs sm:text-sm" >
              <a href={chap.url} className= {`font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'} hover:text-yellow-500 truncate mr-2`}>
                {chap.name}
              </a>
